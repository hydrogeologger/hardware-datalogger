...
def main():    
    ...
    R2 = calculate_theoretical_load_resistor(power = (14.7 * 0.1e-3), I = 0.1e-3)
    print("R2 options: {0}".format(get_common_resistor_value(R2)))
    R2 = get_common_resistor_value(R2)[1]
    
    print("For Design\tR2(Ohm)={0}".format(toSI(R2)))
    print("{0:7s} {1:7s} {2:7s} {3:7s}".format("Vsig(V)","Vsc(V)", "I(A)", "P(Watt)"))
    for Vsigx in Vsig:
        for Vscx in Vsc:
            Ir2 = (Vscx - Vsigx) / R2
            P = R2 * Ir2**2
            print("{0:^7n} {1:>4n}   {2:<8.1e} {3:<7.3f}".format(Vsigx, Vscx, Ir2, P))
